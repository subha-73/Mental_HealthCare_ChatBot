...
def mindfulness():
    title = request.form["title"]
    if title == "Breathing Flower":  # New condition
        return jsonify({
            "description": "Interactive breathing exercise with visual guidance", 
            "file_name": "breathing-exercise"
        })
...
@chatbot.route("/breathing-exercise")
def breathing_exercise():
    """Route for the interactive breathing flower exercise"""
    return render_template("chat/flower.html")

@chatbot.route('/virtual-plant')
def virtual_plant():
    return render_template('virtual_plant.html')

@chatbot.route('/virtual-bunny')
def virtual_bunny():
    return render_template('virtual_bunny.html')
